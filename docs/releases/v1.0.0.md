...
Native macOS voice input inspired by Wispr Flow: press a hotkey, dictate, and PulseScribe inserts the text into the active app.

- **Download:** [PulseScribe-1.0.0.dmg](https://github.com/KLIEBHAN/pulsescribe/releases/download/v1.0.0/PulseScribe-1.0.0.dmg)
...
- **Instant feedback:** menu bar state + animated overlay (~170ms ready time)
- **Auto-paste:** copies to clipboard + sends Cmd+V into the frontmost app

### Multiple transcription providers

- **Deepgram (WebSocket streaming):** transcribes _during recording_ for minimal latency (~300ms)
...
- Add domain terms in `~/.pulsescribe/vocabulary.json` to improve recognition (local + Deepgram)

## App components (what's included)

### Core orchestration

- `pulsescribe_daemon.py`: unified daemon that ties everything together
...
- `transcribe.py`: CLI entry point
  ...
  - daemon mode (SIGUSR1-driven stop)
...
- `pulsescribe_platform/`: macOS-focused platform abstraction (clipboard, sound, app detection)

## Configuration & data locations

PulseScribe stores user data in `~/.pulsescribe/`:

- `~/.pulsescribe/.env`: persistent settings (used by the app + daemon + Settings UI)
- `~/.pulsescribe/logs/pulsescribe.log`: main log file
- `~/.pulsescribe/startup.log`: emergency startup log (early crashes)
- `~/.pulsescribe/vocabulary.json`: custom vocabulary terms

## Install & first run

1. Download [PulseScribe-1.0.0.dmg](https://github.com/KLIEBHAN/pulsescribe/releases/download/v1.0.0/PulseScribe-1.0.0.dmg).
2. Drag **PulseScribe.app** to **/Applications**.
3. Start the app.
4. Grant permissions when prompted:

   - **Microphone** (required)
   - **Accessibility / Input Monitoring** (required for Hold-to-record via pynput and auto-paste)
...
- **MLX limitations:** `mlx-whisper` currently does **not** implement beam search; `PULSESCRIBE_LOCAL_BEAM_SIZE` is ignored on `mlx`.
...
- **ffmpeg:** required for some local _file transcription_ paths; live dictation (mic) does not need ffmpeg.
