...
| Variable                                  | Werte                               | Default          | Beschreibung                                      |
| ----------------------------------------- | ----------------------------------- | ---------------- | ------------------------------------------------- |
| `PULSESCRIBE_WINDOWS_LATENCY_PRESET`      | `snappy`, `safe`, `compat`, `conservative` | `snappy` | Nutzt standardmäßig kurze Windows-Aufnahme-/Finalize-Puffer; Wortenden schützt der adaptive Stop-Nachlauf unabhängig vom Preset. `safe` für konservative Puffer, falls dein Mikrofon trotzdem letzte Wörter abschneidet. Ungültige Werte fallen auf `safe` zurück. |
| `PULSESCRIBE_WINDOWS_STOP_GRACE_SECONDS`  | `0`-`2.0` Sekunden                  | `0.20` (`snappy`, Default), `0.30` (`safe`) | Nimmt unter Windows nach Hotkey-Release kurz weiter auf, damit letzte Wörter nicht abgeschnitten werden. |
| `PULSESCRIBE_WINDOWS_ADAPTIVE_STOP_TAIL`  | `true`, `false`                     | `true` | Verkürzt den Stop-Grace auf ~50 ms, wenn das Audio beim Hotkey-Release bereits ~200 ms still war (fertig gesprochen) – der Text erscheint schneller. Release mitten im Wort behält den vollen Nachlauf. Wird zum Stop-Zeitpunkt ausgewertet und funktioniert daher mit Settings-Reload. |
