...
Gemini 3 models use automatic `thinking_level` optimization for transcript refinement:

| Model                                | Thinking Level | Notes                                        |
| ------------------------------------ | -------------- | -------------------------------------------- |
| **Flash** (`gemini-3-flash-preview`) | `minimal`      | Fastest latency, ideal for quick corrections |
| **Pro** (`gemini-3-pro-preview`)     | `low`          | `minimal` not supported on Pro               |

This is configured automatically based on the model name – no manual configuration needed.
...
| Variable                                  | Values                              | Default          | Description                                         |
| ----------------------------------------- | ----------------------------------- | ---------------- | --------------------------------------------------- |
| `PULSESCRIBE_WINDOWS_LATENCY_PRESET`      | `snappy`, `safe`, `compat`, `conservative` | `snappy` | Uses short Windows capture/finalize buffers by default; the adaptive stop tail protects final words independently of the preset. Set `safe` for conservative buffers if final words still get clipped on your microphone. Invalid values fall back to `safe`. |
| `PULSESCRIBE_WINDOWS_STOP_GRACE_SECONDS`  | `0`-`2.0` seconds                   | `0.20` (`snappy`, default), `0.30` (`safe`) | Keep recording briefly after hotkey release on Windows to avoid clipped final words. |
| `PULSESCRIBE_WINDOWS_ADAPTIVE_STOP_TAIL`  | `true`, `false`                     | `true` | Shorten the stop grace to ~50 ms when the audio tail was already silent for ~200 ms at hotkey release (the speaker finished talking), so text appears faster. Releasing mid-word keeps the full stop grace. Evaluated at stop time, so it works with settings reload. |
| `PULSESCRIBE_WINDOWS_PASTE_SYNC_MS`        | `0`-`500` milliseconds              | `50` | Upper bound for the clipboard sync check before sending Ctrl+V on Windows. PulseScribe pastes as soon as the clipboard read-back confirms the new content (typically <5 ms); this cap only applies when confirmation fails (e.g. clipboard managers/RDP). `0` skips the check entirely. |
| `PULSESCRIBE_WINDOWS_RESPONSIVENESS_BOOST` | `true`, `false`                     | `true` | Applies best-effort process tweaks at daemon startup: 1 ms system timer resolution (`timeBeginPeriod`) for tighter internal polling and `ABOVE_NORMAL` process priority for reliable hotkey/audio handling under load. Requires restart. |
